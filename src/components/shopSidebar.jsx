...
//       {/* Sidebar */}
//       <div
//         className={`fixed lg:relative top-0 left-0 h-full lg:h-auto w-64 lg:w-1/4 p-4 bg-gray-900 text-white border-r border-gray-700 
//           transition-transform duration-300 ease-in-out transform ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"} 
//           lg:translate-x-0 lg:block z-40 shadow-lg overflow-y-auto`}
//       >
//         <h2 className="text-lg font-semibold mb-4 text-yellow-400">Filters</h2>
...
//   const handleSelectAll = () => {
//     if (selectedCategories.size === (Array.isArray(categories) ? categories.length : 0)) {
//       setSelectedCategories(new Set());
//       onFilterChange([]);
//     } else {
...
//             className="mr-2 cursor-pointer accent-yellow-400"
//             onChange={handleSelectAll}
//             checked={selectedCategories.size === (Array.isArray(categories) ? categories.length : 0)}
//           />
//           <label className="font-medium text-white cursor-pointer">All Products</label>
//         </div>
...
const ShopSidebar = ({ categories, onFilterChange }) => {
  ...
  const [expandedCategory, setExpandedCategory] = useState(null);
  const [isSidebarOpen, setIsSidebarOpen] = useState(false);
  const [subcategories, setSubcategories] = useState({}); // Store subcategories by category_id
  ...
  useEffect(() => {
    const fetchSubcategories = async () => {
      try {
        // Assuming an endpoint like GET /api/products/subcategories that returns all subcategories
        ...
        const subcatData = response.data.subcategories || []; // Adjust based on API response
        subcatData.forEach((sub) => {
          if (!subcategoriesMap[sub.categoryId]) {
            subcategoriesMap[sub.categoryId] = [];
          }
          subcategoriesMap[sub.categoryId].push({ subcategoryId: sub.subcategoryId, name: sub.name });
        });
          ...
          <div key={category.id} className="relative mb-4">
            ...
            >
              {(subcategories[category.category_id] || []).map((sub) => (
                <li key={sub.subcategoryId} className="hover:text-yellow-400 cursor-pointer list-none">
                  {sub.name}
                </li>
              ))}
            </div>
            ...
            >
              {(subcategories[category.category_id] || []).map((sub) => (
                <div key={sub.subcategoryId} className="flex items-center">
                  <input
                    type="checkbox"
                    className="mr-2 cursor-pointer accent-yellow-400"
                    onChange={() => handleCategoryChange(sub.name)}
                    checked={selectedCategories.has(sub.name)}
                  />
                  {sub.name}
                </div>
              ))}
